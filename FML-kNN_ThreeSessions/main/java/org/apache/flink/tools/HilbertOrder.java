...
public class HilbertOrder {
	...
	public static String valueOf(int[] coord, ExecConf conf) {
		...
		for (int i=encodedPoints.length-1; i>=0; i--) {
			extra = Functions.createExtra((WORDBITS+10) - Long.toBinaryString(encodedPoints[i]).length());
	...
	public static int[] toCoord(String hilbert, ExecConf conf) {
		...
		String extra = Functions.createExtra(conf.getDimension()*(WORDBITS+10) - bigHStr.length());
		...
		for (int i=conf.getDimension()-1; i>=0; i--) {
			String element = bigHStr.substring(tmp*(WORDBITS+10), (tmp+1)*(WORDBITS+10));
