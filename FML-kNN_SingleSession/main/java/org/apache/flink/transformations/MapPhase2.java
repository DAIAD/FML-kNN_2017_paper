...
public class MapPhase2 extends RichFlatMapFunction<CurveRecord, CurveRecord>
	...
	/** source: http://www.cs.utah.edu/~lifeifei/knnj/#codes **/
        ...
        ArrayList<String> pidList = getPartitionId(input.getFirst(), input.getThird().toString(), input.getFourth().toString());
        if (pidList.size() == 0) {
        ...
        for (int i = 0; i < pidList.size(); i++) {
            ...
            CurveRecord bp2v = new CurveRecord(input.getFirst(), input.getSecond(), input.getThird(), groupKey, input.getClassOrValue());
            output.collect(bp2v);
        }
