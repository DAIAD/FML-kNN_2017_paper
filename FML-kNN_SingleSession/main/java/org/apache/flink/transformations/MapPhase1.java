	...
	public void flatMap(String input, Collector<CurveRecord> output)
		...
		for (int i = 0; i < conf.getShift(); i++) {
			...
			if (conf.getClassifyOrRegress() == 1) {
				output.collect(new CurveRecord(val, id, fileId, i, Float.valueOf(parts[conf.getClassDim()])));
			}
			else if (conf.getClassifyOrRegress() == 2) {
				output.collect(new CurveRecord(val, id, fileId, i, Float.valueOf(parts[conf.getValueDim()])));
			}
		}
	}
}
